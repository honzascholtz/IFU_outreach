...
class Light:
	def __init__(self):
		...
		self.rotor = RotaryEncoder(17, 18,max_steps=30)# pins 17 and 18, max steps = 30
		self.led = PWMLED(22)
	
	def forward(self):
		self.CounterValue = self.CounterValue+10
	...
	def loop(self):
		...
		self.rotor.when_rotated_clockwise = self.forward
		self.rotor.when_rotated_counter_clockwise = self.reverse
		

		while True:
			if self.CounterValue>100:
			...
			if self.NextCounterValue != self.CounterValue:
				print('Counter = %d' % self.CounterValue)
				self.NextCounterValue = self.CounterValue

				
			self.led.value = self.CounterValue / 100.0     # set dc value as the duty cycle
			time.sleep(0.05)
